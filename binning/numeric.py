...
class NumBinner:
    ...
    - иначе — точками выделяются супервстречаемые значения (доля >= point_share),
      остальное бьётся на квантили; точки врезаются в границы, чтобы интервал их не
      накрывал.
    ...
    def fit(self, X) -> Self:
        ...
        # Низкая кардинальность (уникальных <= n_bins): дискретный режим. Точки — только
        # частые значения (>= min_count); редкие уйдут в other при transform.
        if vals.size <= self.n_bins:
            self.points_, self.edges_ = vals[counts >= min_count].tolist(), None
            return self

        # Высокая кардинальность: точка — супервстречаемое значение, проходящее ОБА
        # порога: долю point_share (это спайк) и абсолютный min_count (бин не меньше
        # min_frequency).
        points = []
        ...
        # Границы: внутренние квантили остатка + точки-разделители, края — ±inf.
        # np.unique сортирует и убирает совпадения (точку, попавшую ровно на квантиль).
        # Затем редкие интервалы сливаются ПОСЛЕ врезки точек (точки и ±inf не
        # удаляются), чтобы порог min_count действовал и на «склоны» у точек, а каждый
        # бин был >= min_frequency.
        ...
        self.points_, self.edges_ = (
            points,
            self._merge_rare(to_bin, edges, min_count, points),
        )
        return self
    ...
    def transform(self, X) -> pd.Series:
        ...
        Интервал размечается как ``pd.Interval``, точка — как само число, пропуск — как
        ``NAN_BIN``. В дискретном режиме (edges_ = None) не-точечные и новые значения
        уходят в ``OTHER_BIN``.
    ...
    def sort_categories(X: pd.Series) -> pd.Series:
        ...
        Интервалы идут по возрастанию; точка-число ставится сразу за «своим» интервалом
        (тем, чья правая граница совпадает с точкой), и у этого интервала правая
        граница открывается (closed='neither'); служебные other/missing уходят в конец.
        Значения границ и точек не меняются — только порядок и тип скобки.
        ...
        points = {
            category for category in categories_list if isinstance(category, float)
        }
        ...
        # Порядок: интервал — по левой границе; точка — перед интервалом с той же
        # левой границей.
        ...
        extra_categories = [
            extra for extra in (OTHER_BIN, NAN_BIN) if extra in categories_list
        ]
        return X.cat.reorder_categories(
            sorted_categories + extra_categories, ordered=True
        )
    ...
    def _merge_rare(
        ...
        Порог ``min_count`` считается один раз от полной базы (см. fit) и передаётся
        готовым, чтобы доля min_frequency означала долю всей выборки, а не остатка без
        точек. Защищённые границы — точки и края ±inf — при слиянии не удаляются:
        тонкий «склон» у точки сливается с внешним соседом, сама точка остаётся
        отдельным бином.

        Args:
            X: Непрерывные наблюдения (без точек), по которым считаются размеры
                интервалов.
        ...
        while edges.size > 2:
            ...
            for i in map(int, np.where(X_counts < min_count)[0]):
                # Сливаем интервал, удаляя одну его непрерывную границу: правую, иначе
                # левую.
